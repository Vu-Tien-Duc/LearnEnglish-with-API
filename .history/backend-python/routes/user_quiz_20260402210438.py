...
def submit_quiz():
    ...
    try:
        # ĐÃ SỬA: Ghi đè điểm mới nhất (SET Score = ?) thay vì giữ điểm cao nhất
        cursor.execute("""
            IF EXISTS (SELECT 1 FROM UserLearning WHERE UserID = ? AND WordID = ?)
            BEGIN
                -- Nếu đã tồn tại, cập nhật LearnDate và GHI ĐÈ điểm mới nhất
                UPDATE UserLearning
                SET Score = ?,
                    LearnDate = GETDATE()
        ...
        """, (
            ...
            score,                  # Cho câu lệnh UPDATE (Ghi đè điểm mới)
