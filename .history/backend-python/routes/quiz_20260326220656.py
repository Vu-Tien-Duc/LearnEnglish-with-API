    ...
    try:
        ...
        cursor.execute("""
            SELECT
                ...
                c.CategoryName,
                l.LessonName,                        
                (SELECT COUNT(*) FROM QuizOptions o 
                WHERE o.QuestionID = q.QuestionID) AS OptionCount
            FROM QuizQuestions q
            LEFT JOIN Vocabulary v ON v.WordID = q.WordID
            LEFT JOIN Categories c ON c.CategoryID = v.CategoryID
            LEFT JOIN Lessons l ON l.LessonID = q.LessonID  
...
def create_question():
    try:
        ...
        # Insert câu hỏi
        cursor.execute("""
            INSERT INTO QuizQuestions (WordID, LessonID, QuestionText)
            OUTPUT INSERTED.QuestionID
            VALUES (?, ?, ?)
        """, (word_id, data.get("LessonID", None), q_text))
...
def update_question(question_id):
    try:
        ...
        cursor.execute("""
            ...
            SET WordID=?, LessonID=?, QuestionText=?
            WHERE QuestionID=?
        """, (word_id, data.get("LessonID", None), q_text, question_id))
...
# Thêm endpoint GET lessons (để filter quiz theo lesson)
@quiz_bp.route("/lessons", methods=["GET"])
def get_lessons():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""
