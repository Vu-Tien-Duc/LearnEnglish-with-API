...
def get_users():
    ...
    # ✅ Công thức tiến độ đúng:
    #    Tử số  = số từ đã Mastered của user
    #    Mẫu số = TỔNG từ vựng toàn hệ thống (Vocabulary)
    #    → phản ánh đúng user đã học được bao nhiêu % trong toàn bộ kho từ
    #    NULLIF(..., 0) tránh chia cho 0 khi chưa có từ vựng nào
    cursor.execute(f"""
    ...
    for row in cursor.fetchall():
        ...
        # Online nếu có hoạt động học trong 15 phút gần nhất
        online = "Online" if (
            last_reviewed and
