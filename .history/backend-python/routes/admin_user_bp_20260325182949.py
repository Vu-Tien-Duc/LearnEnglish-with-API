...
def get_users():
    ...
    cursor.execute(f"""
        ...
        LEFT JOIN (
            -- ✅ Cách 3: AVG(điểm CAO NHẤT mỗi từ)
            -- B1: MAX(Score) theo từng cặp (UserID, WordID) → điểm tốt nhất mỗi từ
            -- B2: AVG tất cả điểm tốt nhất đó theo UserID
            -- → không bị kéo xuống bởi những lần làm tệ
            SELECT best.UserID,
                   AVG(CAST(best.BestScore AS FLOAT)) AS Score
            FROM (
                SELECT UserID, WordID, MAX(Score) AS BestScore
                FROM UserLearning
