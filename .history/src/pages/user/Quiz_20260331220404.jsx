...
const Quiz = () => {
  ...
  // State quản lý đúng/sai
  const [isAnsweredCorrectly, setIsAnsweredCorrectly] = useState(false); 
  const [tempWrongOption, setTempWrongOption] = useState(null); // Lưu tạm thời ID đáp án sai để nháy đỏ
  ...
  const resetQuestionState = () => {
    ...
    setTempWrongOption(null); // Reset nút đỏ
  };

  const handleSelectOption = async (option) => {
    // Không cho click nếu đã chọn đúng hoặc đang trong thời gian nháy đỏ nút đó
    if (isAnsweredCorrectly || tempWrongOption === option.id) return;

    if (option.isCorrect) {
      // 1. CHỌN ĐÚNG
      const scoreForThisQ = 100;
      ...
      // 2. CHỌN SAI
      setTotalScore(prev => prev - 25); // Trừ điểm
      setTempWrongOption(option.id); // Set nút này thành đỏ

      // Nút sẽ nháy đỏ trong 600ms (0.6s) rồi tự reset lại thành null (màu trắng)
      setTimeout(() => {
        setTempWrongOption(null);
      }, 600); 
    }
