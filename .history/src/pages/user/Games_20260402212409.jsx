...
export const MemoryGame = () => {
  ...
  // State chứa toàn bộ từ vựng lấy từ API
  const [vocabList, setVocabList] = useState([]);
  const [difficulty, setDifficulty] = useState(null); // null: Chưa chọn mức độ, 6 | 10 | 15: Số từ đang chơi
  ...
  // Lần đầu vào game: Lấy sẵn kho từ vựng
  useEffect(() => {
    setLoading(true);
  ...
  // Xử lý khi user bấm chọn độ khó
  ...
  const initGame = (wordCount) => {
    // Rút ngẫu nhiên đúng số lượng từ theo độ khó
  ...
  const handleCardClick = (index) => {
    ...
    if (newFlipped.length === 2) {
      ...
      if (cards[a].matchId === cards[b].matchId) {
        ...
        setFlippedIndices([]);
        setScore(score + 20); // Chơi vui, chỉ lưu state local
  ...
  return (
    <div style={styles.root}>
      ...
      <div style={styles.gameContent}>
        {loading ? (
          <div style={{textAlign: 'center', marginTop: '80px', fontSize: '20px', fontWeight: 'bold', color: '#16a34a'}}>Đang tải dữ liệu...</div>
        ) : !difficulty ? (
          
          /* MÀN HÌNH CHỌN ĐỘ KHÓ */
          <div style={{backgroundColor: '#fff', padding: '40px', borderRadius: '24px', textAlign: 'center', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', maxWidth: '500px', width: '100%', marginTop: '40px', border: '1px solid #f3f4f6'}}>
            <h2 style={{fontSize: '28px', fontWeight: 800, margin: '0 0 12px', color: '#111827'}}>🧠 Thử thách trí nhớ</h2>
            <p style={{color: '#6b7280', margin: '0 0 32px'}}>Chọn số lượng từ vựng bạn muốn thử sức ở ván này.</p>
            
            <div style={{display: 'flex', flexDirection: 'column', gap: '16px'}}>
              <button onClick={() => handleSelectDifficulty(6)} style={{...styles.btnOutline, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                <span style={{fontSize: '18px', fontWeight: 'bold', color: '#16a34a'}}>🌱 Mức Dễ</span>
                <span style={{color: '#6b7280'}}>6 từ (12 thẻ)</span>
              </button>
              
              <button onClick={() => handleSelectDifficulty(10)} style={{...styles.btnOutline, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                <span style={{fontSize: '18px', fontWeight: 'bold', color: '#ea580c'}}>🔥 Mức Vừa</span>
                <span style={{color: '#6b7280'}}>10 từ (20 thẻ)</span>
              </button>
              
              <button onClick={() => handleSelectDifficulty(15)} style={{...styles.btnOutline, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                <span style={{fontSize: '18px', fontWeight: 'bold', color: '#dc2626'}}>☠️ Mức Khó</span>
                <span style={{color: '#6b7280'}}>15 từ (30 thẻ)</span>
              </button>
            </div>
            
            <button onClick={() => navigate('/mini-game')} style={{background: 'none', border: 'none', color: '#9ca3af', fontSize: '14px', marginTop: '24px', cursor: 'pointer'}}>
              ← Quay lại Menu
            </button>
          </div>
          ...
          <>
            ...
            {isWon && (
              <div style={{backgroundColor: '#fff', padding: '32px', borderRadius: '24px', textAlign: 'center', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', marginBottom: '32px'}}>
                ...
                <p style={{color: '#6b7280', margin: '0 0 24px'}}>Bạn đã hoàn thành mức <strong>{difficulty} từ</strong> với {score} điểm.</p>
                <div style={{display: 'flex', gap: '12px', justifyContent: 'center'}}>
                  <button onClick={() => setDifficulty(null)} style={{...styles.btnSkip, flex: 1}}>Đổi mức khác</button>
                  <button onClick={() => initGame(difficulty)} style={{...styles.btn, backgroundColor: '#16a34a', flex: 2}}>↺ Chơi lại mức này</button>
                </div>
              </div>
            )}
            ...
            <div style={{
              ...styles.memGrid, 
              // Nếu chơi mức khó (30 thẻ), ta thu nhỏ kích thước thẻ một chút để không bị tràn màn hình quá dài
              ...
              {cards.map((card, index) => {
                ...
                // Thu nhỏ font chữ nếu thẻ bị nhỏ lại (Mức khó)
                const fontSizeEn = difficulty >= 15 ? '16px' : '20px';
                const fontSizeVi = difficulty >= 15 ? '13px' : '16px';
                  ...
                  <div key={index} onClick={() => handleCardClick(index)} style={{...styles.memCard, opacity: isMatched ? 0.6 : 1, transform: isMatched ? 'scale(0.95)' : 'none'}}>
                    <div style={{...styles.memInner, transform: isFlipped ? 'rotateY(180deg)' : 'rotateY(0)'}}>
                      ...
                      <div style={{...styles.memBack, backgroundColor: isMatched ? '#f0fdf4' : '#fff', borderColor: isMatched ? '#4ade80' : '#bfdbfe'}}>
                        <span style={{fontWeight: 'bold', fontSize: card.type === 'en' ? fontSizeEn : fontSizeVi, color: card.type === 'en' ? '#1d4ed8' : '#1f2937'}}>
                          {card.text}
                        </span>
                      </div>
                    </div>
...
export const WordScrambleGame = () => {
  const navigate = useNavigate();
  const [allWords, setAllWords] = useState([]); // Lưu toàn bộ từ vựng để dùng cho các ván sau
  const [gameWords, setGameWords] = useState([]); // 10 từ của ván hiện tại
  ...
  // Lần đầu vào game: Tải toàn bộ từ vựng
  useEffect(() => {
    API.get('/user/vocabulary', { params: { user_id: user.UserID } })
      .then(res => {
        // Lọc các từ có độ dài > 2 ký tự để đảo chữ có ý nghĩa
  ...
  // Khởi tạo ván mới (Lấy ngẫu nhiên 10 từ)
  const initNewGame = (sourceList = allWords) => {
    if (sourceList.length === 0) return;
    
    // Xáo trộn mảng và lấy tối đa 10 từ
    ...
    setScore(0);
    setIsGameOver(false);
    setupRound(selected10[0]); // Bắt đầu câu đầu tiên
  };

  // Thiết lập dữ liệu cho 1 câu hỏi cụ thể
  const setupRound = (wordObj) => {
  ...
  const nextRound = () => {
    ...
    if (nextIndex < gameWords.length) {
      ...
      setIsGameOver(true); // Đã hết 10 câu
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (input.toLowerCase() === currentWord.Word.toLowerCase()) {
      ...
      setScore(score + 10); // Mỗi câu đúng 10 điểm
      
      // Đợi 1 giây để user nhìn thấy thông báo đúng rồi mới chuyển câu
      setTimeout(() => {
        nextRound();
      }, 1000);
      ...
      setInput(''); // Xóa ô input cho user nhập lại
    }
  };

  const handleSkip = () => {
    // Bỏ qua câu này, không cộng điểm và sang câu mới luôn
    nextRound();
  };
    ...
    <div style={styles.root}>
      ...
      <div style={styles.gameContent}>
        {loading ? (
          ...
          <div style={{textAlign: 'center', marginTop: '80px', color: '#6b7280'}}>Không đủ từ vựng để chơi!</div>
        ) : isGameOver ? (
          // MÀN HÌNH KẾT THÚC GAME
          <div style={{backgroundColor: '#fff', padding: '40px', borderRadius: '24px', textAlign: 'center', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', maxWidth: '400px', width: '100%', marginTop: '40px'}}>
             <p style={{fontSize: '48px', margin: '0 0 16px'}}>🎯</p>
             <h2 style={{fontSize: '28px', fontWeight: 800, margin: '0 0 8px', color: '#1d4ed8'}}>Hoàn thành!</h2>
             <p style={{color: '#6b7280', margin: '0 0 24px'}}>Bạn đã trả lời xong {gameWords.length} câu hỏi.</p>
             <div style={{fontSize: '40px', fontWeight: 900, color: '#2563eb', marginBottom: '32px'}}>{score} <span style={{fontSize: '16px', color: '#9ca3af', fontWeight: 'normal'}}>Điểm</span></div>
             
             <div style={{display: 'flex', gap: '12px'}}>
               <button onClick={() => navigate('/mini-game')} style={{...styles.btnSkip, flex: 1}}>← Menu</button>
               <button onClick={() => initNewGame()} style={{...styles.btnSubmit, flex: 2}}>↺ Chơi lại 10 từ khác</button>
             </div>
          </div>
        ) : (
              ...
              <div style={styles.scrambleBottom}>
                ...
                <form onSubmit={handleSubmit} style={{display: 'flex', flexDirection: 'column', gap: '16px'}}>
                  <input 
                    ...
                    autoComplete="off" 
                    autoFocus 
                    disabled={message.type === 'success'} // Khóa ô nhập nếu vừa gõ đúng
                  />
