...
import { CheckCircle2, Award, ArrowRight, BookOpen, ChevronRight, ArrowLeft, Sparkles } from 'lucide-react';
...
const Quiz = () => {
  ...
  // Các state quản lý câu hỏi hiện tại
  const [isAnsweredCorrectly, setIsAnsweredCorrectly] = useState(false); 
  const [message, setMessage] = useState(""); // Thông báo khi chọn sai
  ...
  const handleSelectOption = async (option) => {
    // Nếu đã chọn đúng rồi thì không cho click nữa
    if (isAnsweredCorrectly) return;

    if (option.isCorrect) {
      // XỬ LÝ KHI CHỌN ĐÚNG
      const scoreForThisQ = 100; // Cộng 100 điểm cho mỗi câu đúng
      
      setIsAnsweredCorrectly(true);
      setTotalScore(prev => prev + scoreForThisQ);
      setEarnedScoreInfo(`+${scoreForThisQ}`);
      setMessage(""); // Ẩn thông báo lỗi đi nếu có

      try {
        await API.post('/user/quiz/submit', {
          user_id: user.UserID,
          word_id: questions[currentIndex].wordId,
          score: scoreForThisQ, // Gửi điểm lên Python bình thường
        });
      } catch (err) { console.error('Lỗi lưu điểm:', err); }
      
    } else {
      // XỬ LÝ KHI CHỌN SAI
      setTotalScore(prev => prev - 25); // Trừ thẳng 25 điểm vào tổng điểm
      setMessage("❌ Sai rồi! Đã bị trừ 25 điểm. Hãy chọn lại.");
      
      // Không lưu option.id vào state nào cả => Nút không bị đổi màu
            ...
            <div style={styles.questionBox}>
              ...
              <div style={styles.optionsGrid}>
                {questions[currentIndex].options.map((option) => {
                  ...
                  // ĐÃ BỎ LOGIC BÔI ĐỎ NÚT SAI TẠI ĐÂY
                  // Chỉ tô màu xanh khi chọn ĐÚNG
                  ...
                  return (
                    <button
                      ...
                      disabled={isAnsweredCorrectly} // Không disable khi sai, chỉ disable khi đã chọn đúng
                      style={optStyle}
                    >
              ...
              {/* THÔNG BÁO LỖI HIỆN RA KHI CHỌN SAI */}
              {message && !isAnsweredCorrectly && (
...
const styles = {
  ...
  // Style mới cho box thông báo lỗi hiện ở dưới
  errorMessage: { marginTop: '20px', padding: '12px', backgroundColor: '#fee2e2', color: '#991b1b', border: '1px solid #f87171', borderRadius: '12px', textAlign: 'center', fontWeight: 'bold', animation: 'pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite' },
