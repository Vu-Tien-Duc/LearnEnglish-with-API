...
function Navbar({ username, onLogout }) {
  // Sửa lỗi: Nếu chưa login (username = null) thì để trống hoặc hiện icon
    ...
    <nav style={nav.bar}>
      ...
      <div style={nav.right}>
        ...
        {username ? (
          ...
          <span style={nav.name}>Khách</span>
        )}
      </div>
    </nav>
  );
...
export default function Home() {
  ...
  useEffect(() => {
    // 🔥 SỬA Ở ĐÂY: Nếu không có user thì không chuyển hướng, chỉ tắt loading
    if (!user) {
        setLoading(false);
        return; 
    }
    
    // Nếu có user thì mới gọi API lấy thống kê
  ...
  const handleLogout = () => {
    ...
    // Đăng xuất xong f5 lại trang để về trạng thái Khách
  ...
  // 🔥 GIAO DIỆN DÀNH CHO USER ĐÃ ĐĂNG NHẬP (Code cũ của bạn giữ nguyên)
  return (
    <div style={page.root}>
