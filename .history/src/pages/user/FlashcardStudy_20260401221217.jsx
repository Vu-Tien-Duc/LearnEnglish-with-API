...
const FlashcardStudy = () => {
    ...
    const startStudy = async (mode, lesson = null) => {
        ...
        try {
            let res;
            if (mode === 'LESSON') {
                // Đã fix lỗi undefined title bằng cách gọi dự phòng lessonName
                setStudyTitle(`Bài: ${lesson.LessonName || lesson.lessonName}`);
                    ...
                    <div>
                        ...
                        {loading ? <div style={styles.loadingText}>Đang tải...</div> : (
                            <div style={styles.lessonGrid}>
                                {lessons.map((lesson) => (
                                    // Đã fix lỗi undefined cho key, title, tag, và word count
                                    <div key={lesson.LessonID || lesson.lessonId} style={styles.lessonCard} onClick={() => startStudy('LESSON', lesson)}>
                                        <div style={styles.lessonIcon}><BookOpen size={28} /></div>
                                        <div style={styles.lessonInfo}>
                                            <span style={styles.categoryTag}>{lesson.CategoryName || lesson.categoryName}</span>
                                            <h3 style={styles.lessonName}>{lesson.LessonName || lesson.lessonName}</h3>
                                            <p style={styles.wordCount}>{lesson.TotalWords ?? lesson.wordCount} từ vựng</p>
                                        </div>
