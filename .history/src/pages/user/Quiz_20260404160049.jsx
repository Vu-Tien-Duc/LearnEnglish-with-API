...
import { CheckCircle2, XCircle, ArrowRight, BookOpen, ChevronRight, ArrowLeft, Sparkles, Shuffle, RotateCcw, Home } from 'lucide-react';
...
function PageWrap({ children }) {
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: 'DM Sans, sans-serif', display: 'flex', flexDirection: 'column' }}>
      <Navbar />
      <div style={{ flex: 1, maxWidth: 640, margin: '0 auto', padding: '44px 20px 80px', width: '100%' }}>
        {children}
      </div>
      <Footer />
    </div>
...
const Quiz = () => {
  ...
  // Hàm xử lý khi ấn nút quay lại / thoát
  const handleExitQuiz = () => {
    setQuestions([]);
    if (selectedLesson) {
      setView('LESSON_LIST');
      fetchLessons(); // Tải lại để cập nhật điểm TB
    } else {
      setView('MENU');
    }
    ...
    return (
      <PageWrap>
        <PageHeader
          eyebrow="Quiz"
          title="Trắc nghiệm"
          sub="Chọn chế độ làm bài kiểm tra. Trả lời đúng nhận 100đ, sai bị trừ dần điểm."
        />
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          
          <div className="q-lesson-card ac-gold" onClick={openLessonList} style={{ borderColor: '#ecdcb0', background: 'var(--gold-bg)' }}>
            <div style={{
              width: 52, height: 52, borderRadius: 14, flexShrink: 0,
              background: 'var(--white)', border: '1px solid #ecdcb0',
              display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--gold)',
            }}>
              <BookOpen size={24} strokeWidth={1.8} />
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 17, fontWeight: 700, color: 'var(--ink)', marginBottom: 4, fontFamily: 'Playfair Display, serif' }}>
                Học theo Lesson
              </div>
              <div style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.5 }}>
                Làm bài test theo từng chủ đề bài học
              </div>
            </div>
            <ChevronRight size={18} color="var(--border2)" />
          </div>

          <div className="q-lesson-card ac-blue" onClick={() => startQuiz(null)} style={{ borderColor: '#dce7f5', background: 'var(--blue-bg)' }}>
            <div style={{
              width: 52, height: 52, borderRadius: 14, flexShrink: 0,
              background: 'var(--white)', border: '1px solid #dce7f5',
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 17, fontWeight: 700, color: 'var(--ink)', marginBottom: 4, fontFamily: 'Playfair Display, serif' }}>
                Random toàn hệ thống
              </div>
              <div style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.5 }}>
                10 câu hỏi ngẫu nhiên trộn lẫn mọi chủ đề
              </div>
            </div>
            <ChevronRight size={18} color="var(--border2)" />
          </div>
  ...
  // ── VIEW: LESSON LIST (Danh sách bài học) ──────────────────────────────
  if (view === 'LESSON_LIST') {
    return (
      <PageWrap>
        {/* top bar */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32, animation: 'fadeUp 0.4s ease both' }}>
          <button className="back-btn-q" onClick={() => setView('MENU')} style={{ margin: 0 }}>
            ...
            {lessons.map((lesson, i) => (
              ...
              >
                <div style={{
                  ...
                  background: 'var(--gold-bg)', border: '1px solid #ecdcb0',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--gold)',
                }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 12, color: 'var(--muted)' }}>
                    ...
                    {lesson.AvgScore !== null && (
                      <span style={{
                        ...
                        border: '1px solid #b3dcc4', borderRadius: 6, padding: '2px 8px',
  ...
  return (
    <PageWrap>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <button className="back-btn-q" onClick={handleExitQuiz}>
      ...
      {loading ? (
        ...
        <div style={{
          ...
          padding: '48px 36px', boxShadow: 'var(--shadow-lg)', textAlign: 'center',
          animation: 'scaleIn 0.45s cubic-bezier(.22,1,.36,1) both',
        }}>
          <div style={{ fontSize: 52, marginBottom: 16, lineHeight: 1 }}>🎯</div>
          <h2 style={{
            fontFamily: 'Playfair Display, serif', fontSize: 30, fontWeight: 900, color: 'var(--ink)',
          ...
          <div style={{ position: 'relative', display: 'inline-block', marginBottom: 36 }}>
            ...
            <div style={{
              ...
              <div style={{
                ...
                color: avgScore >= 70 ? 'var(--green)' : avgScore >= 40 ? 'var(--gold)' : 'var(--red)', lineHeight: 1,
              }}>{avgScore}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 36 }}>
            <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px' }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 }}>Tổng điểm</div>
            </div>
            <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px' }}>
              <div style={{ fontFamily: 'Playfair Display, serif', fontSize: 26, fontWeight: 900, color: 'var(--ink)', marginBottom: 4 }}>
                {questions.length}
              </div>
              <div style={{ fontSize: 11, color: 'var(--muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 }}>Số câu</div>
            </div>
          </div>

          <div style={{ display: 'flex', gap: 10 }}>
            <button className="q-btn q-btn-outline" style={{ flex: 1, border: 'none' }} onClick={() => startQuiz(selectedLesson)}>
              <RotateCcw size={16} strokeWidth={2} style={{marginRight: 4}}/> Làm lại
            </button>
            <button className="q-btn q-btn-primary" style={{ flex: 1 }} onClick={handleExitQuiz}>
              <Home size={16} strokeWidth={2} style={{marginRight: 4}}/> Bài khác
            </button>
          </div>
        </div>
        ...
        <div style={{ animation: 'fadeUp 0.4s ease both' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            ...
            <div style={{
              display: 'flex', alignItems: 'center', gap: 6, background: 'var(--white)', border: '1px solid var(--border)',
              borderRadius: 99, padding: '5px 14px', fontSize: 12, fontWeight: 700, color: 'var(--ink)',
            }}>
          ...
          {/* question card */}
          <div style={{
            background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 24,
            padding: '36px 32px', boxShadow: 'var(--shadow-md)', marginBottom: 0,
          }}>
            {/* question number tag */}
            <div style={{
              display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 10, fontWeight: 700, letterSpacing: 2.5,
              color: 'var(--muted)', textTransform: 'uppercase', border: '1px solid var(--border)',
            ...
            <h3 style={{
              fontFamily: 'Playfair Display, serif', fontSize: 24, fontWeight: 700, color: 'var(--ink)',
              textAlign: 'center', lineHeight: 1.35, marginBottom: 32,
            }}>
            ...
            {isAnsweredCorrectly && (
              <div style={{
                ...
                marginTop: 28, paddingTop: 24, borderTop: '1px solid var(--border)', animation: 'slideUp 0.3s ease both',
              }}>
                <div style={{
                  display: 'flex', alignItems: 'center', gap: 8, background: 'var(--green-bg)',
                  border: '1px solid #b3dcc4', borderRadius: 10, padding: '8px 16px',
                }}>
