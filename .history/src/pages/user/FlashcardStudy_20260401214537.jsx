...
// THAY ĐỔI ĐỊA CHỈ NÀY CHO KHỚP VỚI FLASK SERVER CỦA BẠN NẾU CẦN
...
// Hàm hỗ trợ ghép đường dẫn cho Ảnh và Audio
const getFullUrl = (path) => {
    if (!path) return null;
    if (path.startsWith('http')) return path; // Nếu đã là link web ngoài thì giữ nguyên
...
const Flashcard = ({ wordInfo, onUpdateProgress, onToggleFavorite }) => {
    ...
    const playAudio = (e) => {
        ...
        // Ưu tiên phát bằng file Audio từ Backend nếu có, nếu không thì dùng AI đọc
        ...
        if (audioUrl) {
            ...
            audio.play().catch((error) => {
                console.warn("⚠️ Không phát được file mp3, chuyển sang AI...", error);
                speakWithAI(); 
            });
        } else {
            speakWithAI();
