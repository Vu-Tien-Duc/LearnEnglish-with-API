...
const playSound = (type) => {
    try {
        ...
        if (type === 'Mastered') {
            // Tiếng "Ting" (Thành công)
            osc.type = 'sine';
            osc.frequency.setValueAtTime(523.25, ctx.currentTime); // Nốt C5
            osc.frequency.exponentialRampToValueAtTime(880, ctx.currentTime + 0.1); // Vuốt lên nốt A5
            gain.gain.setValueAtTime(0.3, ctx.currentTime);
            gain.gain.exponentialRampToValueAtTime(0.01, ctx.currentTime + 0.2);
            osc.start(ctx.currentTime);
            osc.stop(ctx.currentTime + 0.2);
        } else {
            // Tiếng "Swoosh" (Chưa thuộc)
            osc.type = 'triangle';
            osc.frequency.setValueAtTime(250, ctx.currentTime);
            osc.frequency.exponentialRampToValueAtTime(100, ctx.currentTime + 0.15); // Vuốt xuống
            gain.gain.setValueAtTime(0.3, ctx.currentTime);
...
const Flashcard = ({ wordInfo, onUpdateProgress, onToggleFavorite }) => {
    ...
    // State để quản lý hiệu ứng khi bấm nút (X hoặc V)
    const [animating, setAnimating] = useState(null); 

    useEffect(() => {
        setIsFlipped(false);
        setAnimating(null); // Reset hiệu ứng khi chuyển sang từ mới
    ...
    // Hàm xử lý khi bấm nút X hoặc V
    const handleActionClick = (status) => {
        if (animating) return; // Chặn bấm đúp khi đang có hiệu ứng
        
        playSound(status); // Phát âm thanh
        setAnimating(status); // Bật hiệu ứng trượt thẻ
        
        // Đợi 400ms cho hiệu ứng chạy xong rồi mới gọi API và chuyển thẻ
        setTimeout(() => {
            onUpdateProgress(wordInfo, status);
        }, 400); 
    };

    // Style động cho hiệu ứng thẻ trượt đi
    const dynamicCardStyle = {
        ...fcStyles.cardContainer,
        transform: animating === 'Mastered' ? 'translateX(120px) rotate(10deg)' : 
                   animating === 'Learning' ? 'translateX(-120px) rotate(-10deg)' : 'translateX(0) rotate(0)',
    ...
    return (
        <div style={fcStyles.wrapper}>
            {/* Thẻ 3D - Đã thêm dynamic style để trượt */}
            ...
            <div style={{...fcStyles.actionGroup, opacity: animating ? 0.5 : 1, pointerEvents: animating ? 'none' : 'auto'}}>
                <button 
                    ...
                    style={{...fcStyles.btnFail, transform: animating === 'Learning' ? 'scale(0.95)' : 'scale(1)'}}
                >
                    <XCircle size={20} /> Đang học (X)
                </button>
                <button 
                    onClick={() => handleActionClick('Mastered')} 
                    style={{...fcStyles.btnPass, transform: animating === 'Mastered' ? 'scale(0.95)' : 'scale(1)'}}
                >
...
const styles = {
    ...
    resultTitle: { fontSize: '28px', color: '#111827', marginBottom: '30px', fontWeight: 'bold' },
