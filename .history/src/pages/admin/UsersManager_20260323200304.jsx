  ...
  return (
    <div style={{ padding: 32, minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", gap: 8, margin: "16px 0" }}>
        <input placeholder="Tìm kiếm..." value={search} onChange={e => handleSearch(e.target.value)} style={{ flex: 1, padding: 8 }} />
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr>
            ...
            <th style={thStyle}>Quiz TB</th>
            <th style={thStyleRight}>Actions</th>
          </tr>
        </thead>
        <tbody>
          {loading ? (
            <tr><td colSpan={6} style={tdStyle}>Đang tải…</td></tr>
          ) : users.length === 0 ? (
            <tr><td colSpan={6} style={tdStyle}>Không có dữ liệu</td></tr>
          ) : users.map(u => (
            <tr key={u.UserID}>
              <td style={tdStyle}>{u.Username}</td>
              <td style={tdStyle}>{u.Email ?? "-"}</td>
              <td style={tdStyle}>{u.Role}</td>
              <td style={tdStyle}>{u.Progress ?? "-"}</td>
              <td style={tdStyle}>{u.AverageScore ?? "-"}</td>
              <td style={tdStyleRight}>
                ...
                <button className="btn btn-danger btn-sm" onClick={() => handleDelete(u)} disabled={deleting === u.UserID}>
                  {deleting === u.UserID ? "…" : "🗑️"}
                </button>
              </td>
            </tr>
      ...
      {totalPages > 1 && (
        <div style={{ marginTop: 16, display: "flex", gap: 8 }}>
          {Array.from({ length: totalPages }, (_, i) => (
            <button key={i} onClick={() => setPage(i+1)} disabled={i+1 === page}>{i+1}</button>
          ))}
        </div>
      )}
          ...
          <div style={{ width: 420, padding: 16 }}>
            ...
            {modal === "view" && detail && (
              <div style={{ marginTop: 8 }}>
                ...
                <p><b>Quiz TB:</b> {detail.AverageScore ?? "-"}</p>
...
const tdStyleRight = { padding: 8, textAlign: "right" };
