...
// ✅ Self-contained CSS — có thêm LessonID/LessonName
...
const STYLES = `
  ...
  .qm-q-num  { font-family:'DM Serif Display',serif; font-size:18px; color:var(--muted); }
  ...
  .qm-modal {
    background:var(--card); border:1px solid var(--border);
    border-radius:16px; width:620px; max-width:96vw;
  ...
  .qm-fg2 { display:grid; grid-template-columns:1fr 1fr; gap:14px; }
...
function SkelRow() {
  ...
  return (
    <tr>
      {[30, 260, 90, 100, 110, 60, 70].map((w, i) => (
...
export default function QuizManager() {
  ...
  /* ── Fetch vocab + categories + lessons ── */
  ...
  const filtered = questions.filter(q => {
    ...
    const matchLesson  = filterLesson === "all" || String(q.LessonID)   === filterLesson;
    return matchSearch && matchCat && matchWord && matchLesson;
  });
  ...
  // Vocab options filtered by selected category (for filter bar)
  const filteredVocabForBar = filterCat === "all"
    ? vocabList
    : vocabList.filter(v => {
        const cat = categories.find(c => String(c.CategoryID) === filterCat);
        return cat && String(v.CategoryID) === filterCat;
      });
    ...
    <div className="qm">
      ...
      <div className="qm-bar qm-up" style={{ animationDelay: ".05s" }}>
        ...
        >
          ...
          {lessons.map(l => (
            <option key={l.LessonID} value={String(l.LessonID)}>{l.LessonName}</option>
          ))}
        </select>
        <select
          className="qm-input-base qm-select"
                    ...
                    <tr>
                      <td colSpan={7}>
                        <div className="qm-empty">
                          ...
                          <p>{search || filterCat !== "all" || filterWord !== "all" || filterLesson !== "all"
          ...
          <div className="qm-modal">
            ...
            <div className="qm-modal-body">
              ...
              <div className="qm-fg2">
                ...
                <div className="qm-fg">
                  <label className="qm-label">Lesson (tuỳ chọn)</label>
                  <select
                    className="qm-finput"
                    value={form.LessonID ?? ""}
                  ...
                  >
                    ...
                    {lessons.map(l => (
                      <option key={l.LessonID} value={l.LessonID}>
                        {l.LessonName}{l.CategoryName ? ` (${l.CategoryName})` : ""}
                      </option>
                    ))}
                  </select>
                </div>
