...
function SkelRow({ cols = 7 }) {
  const widths = [32, 140, 110, 80, 80, 110, 50];
  return (
    <tr>
      {widths.slice(0, cols).map((w, i) => (
        <td key={i} style={{ padding: "12px 16px" }}>
          <span className="fw-skel" style={{ height: 12, width: w, display: "block" }} />
        </td>
      ))}
    </tr>
  );
  ...
  return (
    <div className="fw-pagination">
      ...
      <div className="fw-pg-controls">
        ...
        {buildPages().map((p, i) =>
          p === "…"
            ? <span key={`dots-${i}`} className="fw-pg-dots">…</span>
            : <button
                key={`page-${p}`}
                className={`fw-pg-btn${p === page ? " active" : ""}`}
...
export default function FavoriteWordManager() {
  ...
  /* ── Derive unique users from favorites list (tránh gọi API sai) ── */
  const uniqueUsers = Array.from(
    new Map(
      favoriteword
        .filter(f => f.UserID != null)
        .map(f => [f.UserID, { UserID: f.UserID, Username: f.Username }])
    ).values()
      ...
      {tab === "list" && (
        <>
          ...
          <div className="fw-filterbar">
            <input
              className="fw-search"
              placeholder="🔍 Tìm từ vựng, user…"
              value={search}
              onChange={e => setSearch(e.target.value)}
            />

            {/* ✅ Fix: dùng uniqueUsers derive từ favoriteword, key dùng UserID string duy nhất */}
            <select
              className="fw-select"
              value={filterUser}
            ...
            >
              ...
              {uniqueUsers.map(u => (
                <option key={`user-${u.UserID}`} value={String(u.UserID)}>
                  {u.Username}
                </option>
              ))}
            </select>
            ...
            {filterUser !== "all" && (
              <button
                className="fw-btn danger"
                onClick={() => {
                  const u = uniqueUsers.find(u => String(u.UserID) === filterUser);
                        ...
                        <tr key={`fav-${item.FavoriteID}`}>
                          ...
                          <td>
                            <div className="fw-actions">
                              <button
                                className="fw-icon-btn del"
                                title="Xóa yêu thích"
                                onClick={() => setConfirmDel({
                                  type: "single",
                                  id: item.FavoriteID,
                                  label: `"${item.Word}" của ${item.Username}`
                                })}
                              >
                                🗑
                              </button>
            ...
            <div className="fw-stats-card-body">
              {loadStats
                ...
                : topWords.length === 0
                  ...
                  : topWords.map((w, i) => (
                    <div key={`top-word-${w.WordID}`} className="fw-stats-row">
            ...
            <div className="fw-stats-card-body">
              {loadStats
                ...
                : userStats.length === 0
                  ? <div style={{ padding: 16, color: "var(--muted)", fontSize: 13 }}>Chưa có dữ liệu.</div>
                  : userStats.map((u, i) => (
                    <div key={`user-stat-${u.UserID}`} className="fw-stats-row">
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <span className="fw-rank">{String(i + 1).padStart(2, "0")}</span>
                        <div className="fw-stats-row-left">
