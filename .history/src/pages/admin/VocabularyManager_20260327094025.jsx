...
const STYLES = `
  ...
  .vm-topbar-row1 h2 { font-family:'DM Serif Display',serif; font-size:22px; line-height:1; }
  ...
  .vm-tbl { width:100%; border-collapse:collapse; font-size:14px; }
  .vm-tbl thead th {
  ...
  .vm-tbl tbody td { padding:10px 14px; vertical-align:middle; }

  /* ── Cell content ── */
  .vm-word-en { font-weight:600; font-size:15px; }
  .vm-word-vi { font-size:13px; color:var(--muted); margin-top:2px; }
  .vm-ipa     { font-size:13px; color:var(--accent2); font-style:italic; }
  .vm-trunc   { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; max-width:160px; color:var(--muted); font-size:13px; }

  /* ── Badges ── */
  .vm-badge { display:inline-flex; align-items:center; padding:3px 9px; border-radius:20px; font-size:11px; font-weight:500; }
  ...
  .vm-accent-tag { display:inline-block; padding:2px 7px; border-radius:20px; font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:.5px; }
  ...
  .vm-thumb  { width:34px; height:34px; border-radius:5px; object-fit:cover; border:1px solid var(--border); display:block; }
  .vm-no-img { width:34px; height:34px; border-radius:5px; border:1px dashed var(--border); display:flex; align-items:center; justify-content:center; color:var(--muted); font-size:13px; }
  .vm-audio  { height:24px; width:110px; filter:invert(1) hue-rotate(145deg) brightness(.8); border-radius:20px; }
  ...
  .vm-pg-info { font-size:12px; color:var(--muted); white-space:nowrap; }
  ...
  .vm-pg-dots { font-size:12px; color:var(--muted); padding:0 3px; line-height:28px; }
  .vm-pg-size { display:flex; align-items:center; gap:6px; font-size:12px; color:var(--muted); }
  .vm-pg-size select {
    background:var(--card); border:1px solid var(--border); border-radius:5px;
    padding:3px 6px; color:var(--text); font-family:'DM Sans',sans-serif;
...
function SkelRow() {
  const S = ({ w="80px" }) => <span className="vm-skel" style={{height:11,width:w,display:"block"}}/>;
  return (
    <tr>
      {[110,70,55,55,45,110,36,140,120,55,44].map((w,i) => (
        <td key={i} style={{padding:"10px 14px"}}><S w={w}/></td>
      ))}
    </tr>
  );
}
        ...
        {buildPages().map((p, i) =>
          p === "…"
            ...
            : (
              <button
                key={p}
                className={`vm-pg-btn${p === page ? " active" : ""}`}
...
export default function VocabularyManager() {
  ...
  // Reset trang 1 mỗi khi filter / pageSize thay đổi
  ...
  const handleDelete = async (id) => {
    try {
      ...
      // Lùi trang nếu trang hiện tại trống sau khi xóa
