  ...
  return (
    <section id="users" style={{ padding: "32px", minHeight: "100vh" }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={u.UserID} style={{ borderBottom: "1px solid #eee" }}>
                ...
                <td style={{ color: "#555" }}>{new Date(u.CreatedDate).toLocaleDateString()}</td>
                <td>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <div style={{ width:80, height:6, background:"#eee", borderRadius:3 }}>
                      <div style={{ width:`${u.Progress ?? 0}%`, height:6, background:"#38bdf8", borderRadius:3 }} />
                    </div>
      ...
      <div style={{ marginTop: 16, display:"flex", justifyContent:"center", gap:12 }}>
        <button onClick={prevPage} disabled={page <= 1} style={{ padding:"6px 12px", cursor: page <= 1 ? "not-allowed" : "pointer" }}>Prev</button>
        <span>Trang {page} / {totalPages}</span>
        <button onClick={nextPage} disabled={page >= totalPages} style={{ padding:"6px 12px", cursor: page >= totalPages ? "not-allowed" : "pointer" }}>Next</button>
      </div>
    </section>
  );
}