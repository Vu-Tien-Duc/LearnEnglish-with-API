...
export default function FavoriteWordManager() {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      const res = await API.get("/admin/favorites/users");
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const [topRes, userRes] = await Promise.all([
        API.get("/admin/favorites/stats/top-words?limit=10"),
        API.get("/admin/favorites/stats/by-user"),
      ]);
  ...
  const handleDeleteSingle = async (favoriteId) => {
    try {
      await API.delete(`/admin/favorites/${favoriteId}`);
  ...
  const handleClearUser = async (userId) => {
    try {
      await API.delete(`/admin/favorites/user/${userId}/clear`);
  ...
  /* ── Tổng số favorites & unique users có fav ── */
  const totalFavs       = favorites.length;
  const uniqueUserCount = new Set(favorites.map(f => f.UserID)).size;
