...
const STYLES = `
  ...
  /* ── Status dot ── */
  .um-status { display:flex; align-items:center; gap:6px; font-size:12px; }
  .um-dot { width:7px; height:7px; border-radius:50%; flex-shrink:0; }
  .um-dot.online  { background:var(--success); box-shadow:0 0 6px rgba(74,222,128,.5); }
  .um-dot.offline { background:var(--muted); }
  ...
  .um-prog-pct   { font-size:11px; color:var(--muted); width:28px; }
...
export default function UsersManager() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    } catch { setMessage("❌ Không tải được danh sách users!"); setUsers([]); }
                ...
                : users.length === 0
                  ...
                  : users.map(u => (
                    <tr key={u.UserID}>
                      ...
                      {/* CreatedDate */}
                      <td style={{ color: "var(--muted)", fontSize: 12 }}>{u.CreatedDate}</td>
                      {/* Progress */}
                      <td>
                      ...
                      <td>
                        <div className="um-status">
                          <span className={`um-dot ${u.Status === "Online" ? "online" : "offline"}`} />
                          <span style={{ color: u.Status === "Online" ? "var(--success)" : "var(--muted)" }}>
                            {u.Status}
                          </span>
                        </div>
                      </td>
            ...
            <div className="um-modal-body">
              <div className="um-form-grid">
                ...
                <div className="um-fg">
                  ...
                  />
                  {editingId && (
                    <span className="um-hint">Chỉ điền nếu muốn đặt lại mật khẩu</span>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="um-overlay" onClick={e => { if (e.target === e.currentTarget) setConfirmDel(null); }}>
          <div className="um-confirm">
            ...
            <p>
              ...
              Toàn bộ tiến độ học, lịch sử quiz và từ yêu thích của user cũng sẽ bị xóa.
            </p>
